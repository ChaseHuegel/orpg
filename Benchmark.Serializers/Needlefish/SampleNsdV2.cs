...
namespace Benchmark.Serializers.Needlefish
{
    public struct TestMessageV2
    {
        ...
        public void SerializeInto(byte[] buffer)
        {
            ...
            // Int
            NeedlefishFormatterV2.WriteHeader(buffer, ref offset, Int_ID, isOptional: false, hasValue: true, isArray: false, arrayLength: 0);
            NeedlefishFormatterV2.Write(buffer, ref offset, Int);

            // OptionalInt
            if (OptionalInt != null)
            {
                NeedlefishFormatterV2.WriteHeader(buffer, ref offset, OptionalInt_ID, isOptional: true, hasValue: true, isArray: false, arrayLength: 0);
                NeedlefishFormatterV2.Write(buffer, ref offset, OptionalInt.Value);
            }

            // Ints
            NeedlefishFormatterV2.WriteHeader(buffer, ref offset, Ints_ID, isOptional: false, hasValue: true, isArray: true, arrayLength: (ushort)(Ints?.Length ?? 0));
            for (int i = 0; i < Ints?.Length; i++)
            {
                NeedlefishFormatterV2.Write(buffer, ref offset, Ints[i]);
            }

            // OptionalInts
            if (OptionalInts != null)
            {
                NeedlefishFormatterV2.WriteHeader(buffer, ref offset, OptionalInts_ID, isOptional: true, hasValue: true, isArray: true, arrayLength: (ushort)OptionalInts.Length);
                for (int i = 0; i < OptionalInts?.Length; i++)
                {
                    NeedlefishFormatterV2.Write(buffer, ref offset, OptionalInts[i]);
                }
            }
